...
database in the server when the server is created without no initial
data.
...
server is created without no initial data.  This can be replaced by
...
During the MySQL server is running, signal handlers for C<SIGINT>,
C<SIGTERM>, and C<SIGQUIT> are installed such that these signal will
terminate the server.  If your application want to handle these
signals in other ways (e.g. terminating entire your application),
another signal handlers should also be installed using L<AnyEvent> API
(or equivalent).
